...
BEGIN
    ...
    BEGIN TRY
      ...
      BEGIN
        ...
        BEGIN
          ...
          UPDATE cmms_auth.Users SET Department=@Department,Username=@Username,DisplayName=@DisplayName,RoleCode=@Role,IsActive=@Active,Version=@Version,PasswordHash=@Hash,UpdatedAt=SYSUTCDATETIME() WHERE UserId=@Id;
        END
        ELSE
        BEGIN
  ...
  END;
  SELECT (SELECT
    JSON_QUERY((SELECT Department AS department,UserId AS id,Username AS username,DisplayName AS displayName,RoleCode AS role,IsActive AS active,Version AS version,PasswordHash AS passwordHash FROM cmms_auth.Users ORDER BY Username FOR JSON PATH)) AS users,
