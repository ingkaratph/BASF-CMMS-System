...
export function MediaGallery({resource,id,role,row}:{resource:Resource;id:string;role:Role|undefined;row:Row}){
 ...
 const writable=['ADMINISTRATOR','PLANNER'].includes(role||'')&&canWrite(role,'PUT',resource,row);
 ...
 async function upload(files:FileList|null){
  ...
  try{
   const selected=[...files],maxImages=resource==='assets'?5:1;
 ...
 return <section className="media-gallery"><div className="media-heading"><div><h3>รูปภาพ{resource==='assets'?'และเอกสารเครื่องจักร':''}</h3><p>{images.length} / {resource==='assets'?5:1} ภาพ · เก็บไฟล์ในเครื่องเซิร์ฟเวอร์ CMMS</p></div>{writable&&<label className={`button primary upload-button ${busy?'is-busy':''}`}><Upload size={16}/>{busy?'กำลังอัปโหลด…':'อัปโหลดไฟล์'}<input aria-label="อัปโหลดรูปหรือเอกสาร" type="file" disabled={busy} multiple={resource==='assets'} accept={resource==='assets'?'.jpg,.jpeg,.png,.webp,.pdf,.dwg,.dxf':'.jpg,.jpeg,.png,.webp'} onChange={e=>{upload(e.target.files);e.target.value=''}}/></label>}</div>
 {error&&<p className="error-box" role="alert">{error}</p>}{notice&&<p role="status">{notice}</p>}
 <div className="media-grid">{images.map((item,index)=><figure key={item.id}><button className="media-photo" aria-label={`ดูภาพที่ ${index+1}`} onClick={()=>setViewer(index)}><img src={item.url} alt={`ภาพที่ ${index+1}`}/></button>{writable&&<figcaption><button className="icon-button" disabled={busy} aria-label={`ลบภาพที่ ${index+1}`} onClick={()=>remove(item)}><Trash2 size={15}/></button></figcaption>}</figure>)}
 ...
 {documents.length>0&&<ul className="media-documents">{documents.map(item=><li key={item.id}><FileText size={20}/><a href={item.url}>{item.name}<small>{(item.size/1024).toFixed(0)} KB · {item.createdBy}</small></a>{writable&&<button className="icon-button" disabled={busy} aria-label={'ลบ '+item.name} onClick={()=>remove(item)}><Trash2 size={16}/></button>}</li>)}</ul>}
 <p className="media-help">{resource==='assets'?'JPG, PNG, WebP · เอกสาร PDF, DWG, DXF':'JPG, PNG, WebP'} · สูงสุด 15 MB ต่อไฟล์{writable?'':' · อัปโหลดได้เฉพาะ Planner / Administrator ที่มีสิทธิ์แก้ไขทะเบียน'}</p></section>;
}
