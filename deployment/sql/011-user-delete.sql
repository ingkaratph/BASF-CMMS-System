IF COL_LENGTH('cmms_auth.Users','DeletedAt') IS NULL ALTER TABLE cmms_auth.Users ADD DeletedAt nvarchar(40) NULL;
GO

ALTER PROCEDURE cmms_auth.usp_IdentityStorage
...
BEGIN
    ...
    BEGIN TRY
      ...
      BEGIN
        ...
        DECLARE @Id varchar(24)=JSON_VALUE(@BodyJson,'$.user.id'),
          ...
          @DeletedAt nvarchar(40)=JSON_VALUE(@BodyJson,'$.user.deletedAt'),
          @Department nvarchar(50)=JSON_VALUE(@BodyJson,'$.user.department'),
        ...
        IF @Id=@Actor AND EXISTS(SELECT 1 FROM cmms_auth.Users WHERE UserId=@Id AND RoleCode='ADMINISTRATOR') AND (@Active=0 OR @Role<>'ADMINISTRATOR') THROW 51000,'Cannot disable own administrator',1;
        ...
        BEGIN
          ...
          UPDATE cmms_auth.Users SET DeletedAt=@DeletedAt,Department=@Department,Username=@Username,DisplayName=@DisplayName,RoleCode=@Role,IsActive=@Active,Version=@Version,PasswordHash=@Hash,UpdatedAt=SYSUTCDATETIME() WHERE UserId=@Id;
        END
        ELSE
        BEGIN
          IF @Version<>1 THROW 51000,'Invalid initial version',1;
          INSERT cmms_auth.Users(Department,UserId,Username,DisplayName,RoleCode,IsActive,Version,PasswordHash) VALUES(@Department,@Id,@Username,@DisplayName,@Role,@Active,@Version,@Hash);
        END;
  ...
  END;
  SELECT (SELECT
    JSON_QUERY((SELECT DeletedAt AS deletedAt,Department AS department,UserId AS id,Username AS username,DisplayName AS displayName,RoleCode AS role,IsActive AS active,Version AS version,PasswordHash AS passwordHash FROM cmms_auth.Users ORDER BY Username FOR JSON PATH)) AS users,
